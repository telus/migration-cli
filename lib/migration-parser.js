...
const payloadBuilder = require('./migration-payloads');
...
const createMigrationParser = function (makeRequest, hooks) {
  ...
  return Bluebird.coroutine(function * migration (migrationCreator) {
    ...
    const entryMigrationPlan = plan.filter((chunk) => {
    ...
    const planWithPayloads = payloadBuilder(plan, contentTypes, entries);
    ...
    const requests = buildRequests(planWithPayloads, contentTypes);
