...
module.exports = function payloadBuilder (chunk, state) {
  ...
  if (state.fieldIdChanges[id]) {
    updatedCt.meta.parent = state.fieldIdChanges[id];
    delete state.fieldIdChanges[id];
  } else if (contentType) {
  ...
  if (hasFields(basisForNextPayload)) {
    updateFieldIdsForChanges(basisForNextPayload, state.fieldIdChanges);
  }
