...
function buildFields (result, actions) {
  ...
  actions.forEach((action) => {
    ...
    if (action.type === 'field/validations') {
      const source = _.find(result.payload.fields, { id });

      // TODO: Handle other scenarios
      source.items.validations[0].linkContentType = source.items.validations[0].linkContentType.concat(props.items.validations[0].linkContentType);
    }
