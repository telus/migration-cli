...
module.exports = function (chunks) {
  return chunks.map((chunk) => {
    ...
    const displayFieldValidation = checkIfDisplayFieldsExist(chunk);
    const ctErrors = validateContentType(chunk);
    const fieldErrors = validateFields(chunk);
    const deletionErrors = checkIfDeletionWasOmitted(chunk);
    const typeChangeErrors = checkIfTypeWasChanged(chunk);
